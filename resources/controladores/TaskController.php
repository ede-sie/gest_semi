...
function createTask($data) { 
    ...
    $stat  = $conn->real_escape_string($data['status']);
    $prio  = $conn->real_escape_string($data['priority']); 
    $dead  = $conn->real_escape_string($data['deadline']); 
 
    $sql = "INSERT INTO tasks(title, description, user_id, status, priority, deadline, assigned_at) 
            VALUES ('$title', '$desc', $user, '$stat', '$prio', '$dead', NOW())"; 
    return $conn->query($sql); 
} 
